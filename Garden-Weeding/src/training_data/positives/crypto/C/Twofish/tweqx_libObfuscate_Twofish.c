...
static const uint8_t tab_5b[4] = {0, G_M >> 2, G_M >> 1,
                                  (G_M >> 1) ^ (G_M >> 2)};
static const uint8_t tab_ef[4] = {0, (G_M >> 1) ^ (G_M >> 2), G_M >> 1,
                                  G_M >> 2};
...
static const uint8_t ror4[16] = {0, 8,  1, 9,  2, 10, 3, 11,
                                 4, 12, 5, 13, 6, 14, 7, 15};
static const uint8_t ashx[16] = {0, 9, 2,  11, 4,  13, 6,  15,
                                 8, 1, 10, 3,  12, 5,  14, 7};

static const uint8_t qt0[2][16] = {
    {8, 1, 7, 13, 6, 15, 3, 2, 0, 11, 5, 9, 14, 12, 10, 4},
    {2, 8, 11, 13, 15, 7, 6, 14, 3, 1, 9, 4, 0, 10, 12, 5}};

static const uint8_t qt1[2][16] = {
    {14, 12, 11, 8, 1, 2, 3, 5, 15, 4, 10, 6, 7, 0, 9, 13},
    {1, 14, 2, 11, 4, 12, 3, 7, 6, 13, 10, 5, 15, 9, 0, 8}};

static const uint8_t qt2[2][16] = {
    {11, 10, 5, 14, 6, 13, 9, 0, 12, 8, 15, 3, 2, 4, 7, 1},
    {4, 12, 7, 5, 1, 6, 9, 10, 0, 14, 13, 8, 2, 11, 3, 15}};

static const uint8_t qt3[2][16] = {
...
static uint8_t qp(const uint32_t n, const uint8_t x) {
  ...
  uint8_t b0 = x & 15;
  uint8_t a1 = a0 ^ b0;
  uint8_t b1 = ror4[b0] ^ ashx[a0];
  uint8_t a2 = qt0[n][a1];
  uint8_t b2 = qt1[n][b1];
  uint8_t a3 = a2 ^ b2;
  uint8_t b3 = ror4[b2] ^ ashx[a2];
  uint8_t a4 = qt2[n][a3];
  uint8_t b4 = qt3[n][b3];
...
static void gen_qtab(TWOFISH_DATA *pTfd) {
...
static void gen_mtab(TWOFISH_DATA *pTfd) {
  for (uint32_t i = 0; i < 256; ++i) {
    uint32_t f01 = q(1, i);
    uint32_t f5b = ffm_5b(f01);
    uint32_t fef = ffm_ef(f01);
...
static uint32_t h_fun(TWOFISH_DATA *pTfd, const uint32_t x,
                      const uint32_t *key) {
  uint32_t b0 = byte(x, 0);
  uint32_t b1 = byte(x, 1);
  uint32_t b2 = byte(x, 2);
  uint32_t b3 = byte(x, 3);
  ...
  b2 = q(1, b2);
  b3 = q(0, b3);
  uint32_t m5b_b0 = ffm_5b(b0);
  uint32_t m5b_b1 = ffm_5b(b1);
  uint32_t m5b_b2 = ffm_5b(b2);
  uint32_t m5b_b3 = ffm_5b(b3);
  uint32_t mef_b0 = ffm_ef(b0);
  uint32_t mef_b1 = ffm_ef(b1);
  uint32_t mef_b2 = ffm_ef(b2);
  uint32_t mef_b3 = ffm_ef(b3);
...
static void gen_mk_tab(TWOFISH_DATA *pTfd, uint32_t *key) {
  switch (pTfd->k_len) {
  case 2:
    for (uint32_t i = 0; i < 256; ++i) {
      uint8_t by = i;
...
#ifdef ONE_STEP
      pTfd->mk_tab[0][i] = mds(0, q20(by));
      pTfd->mk_tab[1][i] = mds(1, q21(by));
      pTfd->mk_tab[2][i] = mds(2, q22(by));
      pTfd->mk_tab[3][i] = mds(3, q23(by));
#else
      sb[0][i] = q20(by);
      sb[1][i] = q21(by);
      sb[2][i] = q22(by);
...
#ifdef ONE_STEP
      pTfd->mk_tab[0][i] = mds(0, q30(by));
      pTfd->mk_tab[1][i] = mds(1, q31(by));
      pTfd->mk_tab[2][i] = mds(2, q32(by));
      pTfd->mk_tab[3][i] = mds(3, q33(by));
#else
      sb[0][i] = q30(by);
      sb[1][i] = q31(by);
      sb[2][i] = q32(by);
...
#ifdef ONE_STEP
      pTfd->mk_tab[0][i] = mds(0, q40(by));
      pTfd->mk_tab[1][i] = mds(1, q41(by));
      pTfd->mk_tab[2][i] = mds(2, q42(by));
      pTfd->mk_tab[3][i] = mds(3, q43(by));
#else
      sb[0][i] = q40(by);
      sb[1][i] = q41(by);
      sb[2][i] = q42(by);
...
#ifdef ONE_STEP
#define g0_fun(x)                                                              \
  (pTfd->mk_tab[0][byte(x, 0)] ^ pTfd->mk_tab[1][byte(x, 1)] ^                 \
   pTfd->mk_tab[2][byte(x, 2)] ^ pTfd->mk_tab[3][byte(x, 3)])
#define g1_fun(x)                                                              \
  (pTfd->mk_tab[0][byte(x, 3)] ^ pTfd->mk_tab[1][byte(x, 0)] ^                 \
...
static uint32_t mds_rem(uint32_t p0, uint32_t p1) {
  for (uint32_t i = 0; i < 8; ++i) {
    uint32_t t = p1 >> 24; // get most significant coefficient
...
void Twofish_set_key(TWOFISH_DATA *pTfd, const uint32_t *in_key,
  ...
  for (uint32_t i = 0; i < pTfd->k_len; ++i) {
    uint32_t a = in_key[i + i];
    me_key[i] = a;

    uint32_t b = in_key[i + i + 1];
    mo_key[i] = b;
...
void Twofish_encrypt(const TWOFISH_DATA *pTfd, const uint32_t *in_blk,
                     uint32_t *out_blk) {
  uint32_t blk[4];
...
void Twofish_decrypt(const TWOFISH_DATA *pTfd, const uint32_t *in_blk,
                     uint32_t *out_blk) {
  uint32_t blk[4];
