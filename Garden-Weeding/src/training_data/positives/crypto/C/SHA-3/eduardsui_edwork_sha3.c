...
void sha3_256(const void *in, size_t len, void *hash) {
    sha3_context ctx;
    sha3_Init256(&ctx);
    sha3_Init256(&ctx);
    sha3_Update(&ctx, in, len);
    memcpy(hash, sha3_Finalize(&ctx), 32);
}

void hmac_sha3_256(const void *key_gc, size_t key_len, const void *data, size_t data_len, void *hash) {
    sha3_context sha;
    ...
    if (key_len > 64)
        sha3_256(key_gc, key_len, key);
    else
    if (key_len < 64) {
        memcpy(key, key_gc, key_len);
        memset(key + key_len, 0, 64 - key_len);
    }
    for (i = 0; i < 64; i++) {
        okey[i] = key[i] ^ 0x5C;
        ikey[i] = key[i] ^ 0x36;
    ...
    sha3_Init256(&sha);
    sha3_Update(&sha, ikey, 64);
    sha3_Update(&sha, data, data_len);
    memcpy(temp_hash, sha3_Finalize(&sha), 32);

    sha3_Init256(&sha);
    sha3_Update(&sha, okey, 64);
    sha3_Update(&sha, temp_hash, 32);
    memcpy(hash, sha3_Finalize(&sha), 32);
}
